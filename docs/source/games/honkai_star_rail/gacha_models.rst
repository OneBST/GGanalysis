...
GGanalysis 使用基本的抽卡模板模型结合 `崩坏：星穹铁道抽卡系统初步分析参数 <https://www.bilibili.com/video/BV1sM411G7NF/>`_ 定义了一系列可以直接取用的抽卡模型。

.. attention:: 

   崩坏：星穹铁道的四星保底不会被五星重置，但与五星耦合时仍会在综合概率上产生细微的影响。此处的模型没有考虑四星和五星的耦合。

   崩坏：星穹铁道常驻跃迁中具有“平稳机制”，即角色和光锥两种类型的保底，GGanalysis 包没有提供这类模型，有需要可以使用 `GGanalysislib 包 <https://github.com/OneBST/GGanalysislib>`_ 。角色活动跃迁及光锥活动跃迁中也有此类机制，由于其在“UP机制”后生效，对于四星UP道具抽取可忽略。
