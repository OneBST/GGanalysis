...
class HardTypePityDP():
    ...
    def __call__(self, item_pity=0, type_pity=0) -> np.ndarray:
        # 根据保底情况计算极限位置
        calc_pulls = len(self.item_pull_dist)-1 + self.type_pity_gap*self.item_types - type_pity + self.type_pull_shift
        # 状态定义
        ...
        for i in range(1, calc_pulls+1):
            ...
            for j in range(max(0, i-self.item_pity_pos), i):
                ...
                if max(i-1+type_pity-self.type_pull_shift, 0)//self.type_pity_gap != max(j-1+type_pity-self.type_pull_shift, 0)//self.type_pity_gap:
...
class AKHardPityModel(CommonGachaModel):
    ...
    def __call__(self, item_num: int=1, multi_dist: bool=False, item_pity=0, type_pity=0) -> Union[FiniteDist, list]:
        if not multi_dist:
...
class AKDirectionalModel(CommonGachaModel):
    ...
    def _get_dist(self, item_num, item_pity, type_pity):
        ...
        c_dist.dist[:self.type_pity_gap+self.type_pull_shift-type_pity+1] = 0
        # 其他保底的概率分布 (没有归一化)
        f_dist = FiniteDist(self.DP_module(0, 0))
        f_dist.dist[:self.type_pity_gap+self.type_pull_shift+1] = 0
        # 第一个不保底的概率分布 (没有归一化)
        first_lucky = FiniteDist(self.DP_module(item_pity, type_pity)[:self.type_pity_gap+self.type_pull_shift-type_pity+1])
        # 随后不保底的概率分布 (没有归一化)
...
single_up_6star = AKDirectionalModel(single_up_6star_old(1), p2dist(PITY_6STAR), type_pity_gap=150, item_types=1, up_rate=0.5)
