...
class MarkovTransition():
    ...
    def first_hitting_time(
        ...
        f[t] = P(首次在物理时间 t 命中 target), t = 0..steps
        surv = P(在 0..steps 内始终未命中)
        '''
        ...
        hit = self.space.compile_hitter(target)
    ...
    def stationary(
        self,
        method: Literal["power", "eigs", "solve"] = "power",   
        tol: float = 1e-12,
        max_iter: int = 200_000,
        lazy: float = 0.0,
        x0: Optional[np.ndarray] = None,
    ) -> np.ndarray:
        # 计算平稳分布 pi 满足 pi = P @ pi , sum(pi) = 1
        method = method.lower()
        if method == "power":
            return self._stationary_power(tol, max_iter, lazy, x0)
        if method == "eigs":
            return self._stationary_eigs()
        if method == "solve":
            return self._stationary_solve()
        raise ValueError("method must be power|eigs|solve")
    ...
    def _stationary_power(
        ...
        if self.backend == "dense":
            return p
        if lazy != 0.0 and not (0.0 < lazy < 1.0):  # 惰性随机游走（lazy random walk）”的权重，用来减慢每一步的移动，从而提高收敛稳定性。
    ...
    def _stationary_eigs(self) -> np.ndarray:
        '''
        使用特征值分解求平稳分布。
        '''
        if spla is None:
        ...
        if self.backend == "dense":
            ...
            w, v = spla.eigs(self.P, k=1, which="LM")
            vec = np.real(v[:, 0])
    ...
    def _stationary_solve(self) -> np.ndarray:
        '''
        解线性方程 (I - P)pi = 0 ，并加约束 sum(pi)=1。
        '''
        if sp is None:
